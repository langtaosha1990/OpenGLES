...
typedef struct {
    ...
    GLint mapLoc0;      // 纹理对应的location
    GLuint mapTexId0;   // 纹理缓冲区id
    
    //
    GLint mapLoc1;
    GLuint mapTexId1;   // 纹理缓冲区id
...
void Update(ESContext *esContext, float deltaTime)
{
    ...
    /*
     ...
     参数5：远平面距离
     */
    esPerspective(&perspective, 30.0f, aspect, 3.0f, 15.0f);
    
    // 初始化模型矩阵
    esMatrixLoadIdentity(&modelview);
    // 将模型矩阵沿z轴移动-5.0个单位
    esTranslate(&modelview, -1.0, 1.0, -12.0);
